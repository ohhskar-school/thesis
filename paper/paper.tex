...
\begin{figure}[H]
	...
	\caption{Logitech C270. Reprinted from \fullcite{logitech}}
	\label{fig:metho-cam}
	\centering
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[font=\small,thick]
		...
		\node[draw,
			below=of input,
			minimum width=3.5cm,
			minimum height=1cm] (bg-rem) {Background Removal};
	...
	\label{fig:metho-key-algo}
\end{figure}


\subsubsection{Background Removal}
The background of the frame will be removed by using colors as the main metric
for determining the foreground and the background. This is made easier by the
setup as the setup will have a green background, with a light colored keyboard
as a foreground.
...
MediaPipe Hands. This algorithm is composed of multiple ML models working in
...
using a single shot detector model based on the paper by \cite{ssd}. This
model achieves an average precision of 95.7\% in palm detection.
...
self-occlusion. This is also made possible by the model's consistent internal hand
pose representation.
