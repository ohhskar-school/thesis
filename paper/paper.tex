...
\usepackage{hyperref}
\hypersetup{
	pdftitle={Development of a touch typing trainer with an emphasis on finger-key identification},
	pdfstartview={FitH},
	colorlinks=true,
	linkcolor=black,
	citecolor=black,
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[font=\small,thick]
		...
		\node[draw,
			...
			minimum height=1cm,
			inner sep = 3mm
		] (map) {Overlay Virtual Keyboard Map using Contour Points};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[font=\small,thick]
		...
		\node[draw,
			...
			align = center,
			inner sep = 3mm
		] (map) {Virtual Keyboard Map \\ Finger Tracking Data};
		...
		\node[draw,
			...
			minimum height = 1cm,
			inner sep = 3mm
		] (find-roi) {Find \ac{roi} of the pressed key in Virtual Keyboard Map};
...
\subsubsection{Find \ac{roi} of the pressed key in Virtual Keyboard Map}
Since there is a known color-key mapping based on the Virtual Keyboard Map, the
algorithm will find the correct color to search for based on the received key
press packet. This color is isolated from the Virtual Keyboard Map, and the
\ac{roi} and its coordinates are obtained. The edges of this singular \ac{roi}
will then be obtained by finding its contours. The specific implementation of
this is explained in Section~\ref{section:metho-algo-key-contours}.
